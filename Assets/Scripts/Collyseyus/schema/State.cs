...
public partial class State : Schema {
	...
	[Type(5, "number")]
	public float boardSize = default(float);
	...
	protected event PropertyChangeHandler<float> __boardSizeChange;
	public Action OnBoardSizeChange(PropertyChangeHandler<float> __handler, bool __immediate = true) {
		if (__callbacks == null) { __callbacks = new SchemaCallbacks(); }
		__callbacks.AddPropertyCallback(nameof(this.boardSize));
		__boardSizeChange += __handler;
		if (__immediate && this.boardSize != default(float)) { __handler(this.boardSize, default(float)); }
		return () => {
	...
	protected override void TriggerFieldChange(DataChange change) {
		switch (change.Field) {
			...
			case nameof(boardSize): __boardSizeChange?.Invoke((float) change.Value, (float) change.PreviousValue); break;
			default: break;
		}
	}
}
